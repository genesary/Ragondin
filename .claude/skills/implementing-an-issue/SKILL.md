...
## Why the design phase is already done
...
1. **Work in your own worktree, before anything else.** `git worktree add <path> -b <type>/<issue-number>-<slug> origin/main`. The checkout you were handed may be shared with other agents, and a `git checkout -b` there moves `HEAD` under every one of them mid-task; `docs/AGENT_WORKFLOW.md` § *One issue, one branch, one worktree, one agent* says why the mapping is strict. If you find the tree already dirty or already on someone else's branch: never `git add -A`, `git commit -a`, `git stash` or `git reset --hard`, and never `git checkout`, `git restore` or `git reset` a file you do not own — those are someone's uncommitted work. Commit path-limited, naming your files explicitly, and confirm `git show --stat HEAD` lists only them before you push.
2. **The design is settled.** Your specification is the issue, plus `AGENTS.md` and `docs/`. Do **not** run a brainstorming or design phase to re-derive decisions already made. If the issue tells you what to build, build that.
3. **Read the crate's `ARCHITECTURE.md` before modifying it.** Each load-bearing crate states its local invariants there. Load the `architecture-invariants` skill too, before writing code.
4. **Test-driven, strictly.** Write a failing test first; implement the minimum to make it pass; then refactor. A test that passes without exercising the behavior is worse than no test.
5. **Stay inside Scope — IN; honor Scope — OUT.** Scope — OUT exists to prevent collisions with other agents working in parallel on disjoint crates. Touching a file the issue puts out of scope is not a favor; it is a merge conflict and a boundary violation. The one exception is documentation your own diff makes necessary or false, which `AGENTS.md` § *Documentation ships with the code it describes* puts in scope whatever the issue says.
...
8. **If an architectural choice is required that `AGENTS.md` § Rules of engagement says escalates** — that section lists the shared surfaces, and says what a choice that does not escalate owes instead: **stop and open a `decision` issue** (see the `opening-a-decision-issue` skill). Do not guess. An agent that pauses on an ambiguity costs minutes; an agent that guesses an architectural decision costs a refactor. The same applies if you are about to reopen a settled decision — load the `frozen-decisions` skill and take the path it gives you.
