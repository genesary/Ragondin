...
5. **Let the composition root construct it.** The binary — the composition root — constructs your component explicitly. A node family (the tables `docs/code-architecture.md` § 8.1 lists) is registered on the `EngineContext`; an embedder or a vector store has no table there and is built inside the constructor the root registers for the dense retriever (ADR-C32 § 3). Either way construction is something the composition root's code does, never something your crate arranges for itself at load time. **INV-6** (`AGENTS.md` § Invariants) states what that rules out and how much of it CI decides; the **Registry** row of § Frozen decisions says why it was chosen.
