...
A constructor receives the node's **configuration** — `ragondin-pipeline`'s untyped `Params` (`BTreeMap<String, ParamValue>`), or the subset of it planning resolves — and never a per-call params struct. `RerankParams` carries only `top_k` (§7.1), which is not enough to construct anything: what a reranker needs at construction is a model path, a device, an ONNX session. The two kinds of parameter travel by different routes, and §6.3 is where they part.
