...
answer. #263's scope said `BeirAdapter` does not change; this decision amends
that scope to carry this reading path.
...
2) and runs its generation configurations under `--benchmark beir-qa/dataset`;
the M2 runs, still loaded as `beir/dataset`, stay reference-free and unchanged.
The gap that criterion demonstrates is there by construction, and nothing
licensed enters the tree for it.
...
built with an empty reference-answers value carries none. Every use of the word
in this ADR — the regime, the walk of § 3, the refusal and the digest below —
...
- **The M3 evaluation issues are unblocked, and each owes this ADR something
  ...
  Consequences named #263 for that obligation. #268: an `answers.jsonl` of
  hand-written reference answers in the M2 exit-criterion dataset, and its
  generation configurations run under `beir-qa/dataset` while the M2 runs stay
  under `beir/dataset`. #269: the SciFact assertion through the new read path,
  the SQuAD calibration, the subset rule with its floor of N ≥ 1 000, the
  tolerances of § 4, and the frozen fixture with its licence notice. None of
  them reopens this decision; each implements a named part of it.
