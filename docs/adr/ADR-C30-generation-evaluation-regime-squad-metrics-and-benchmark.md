...
**What SQuAD v1.1 dev offers.** Its paragraphs are mostly short enough for the
encoder:
...
ADR-10's own Amendments section records as the thing not to repeat.
Decided in #252.
...
counterpart.** It extends ADR-10's Decision, as ADR-10's 2026-09-05 amendment
did for `trec_eval` — but by a separate ADR, which is the route that amendment
itself prescribes — and it reopens nothing (process rule 4 of
...
`pytrec_eval_parity.tsv` is regenerated by one that calls `pytrec_eval`. The
regeneration script records the Python version it ran under, because the Unicode
tables behind `str.lower` and `\b` change between versions; this ADR pins
Python 3, not a minor version. The fixture covers at least:
...
- **The corpus is the paragraphs.** Each paragraph is one `Document`; its
  ...
  article title under the key `title` — a key, like its value, that
  `dataset_version` digests.
...
a corrupt or wrong file. The adapter's own tests (#263) run over a miniature
fixture in the tree, which may be hand-written or a tiny subset of the dev file;
a subset carries the CC BY-SA 4.0 notice beside it, as every fixture quoting the
dataset does.

**A BEIR directory may carry reference answers too**, as the fourth piece in
BEIR's own id scheme: a file `answers.jsonl` beside the directory's other files,
one line per query, `{"_id": <BEIR query id>, "answers": [<strings>]}`, the
strings in the order the line gives them. `--benchmark beir/<dir>` ignores that
file and stays reference-free, loading the directory exactly as M2 does, so no
M2 run is refused under § 5. `--benchmark beir-qa/<dir>` requires the file and
loads the references from it; a query with no line in it carries no reference
answer. Two cases are adapter errors, each a `BenchmarkError` naming the file
and the id: a line whose `_id` names no query in `queries.jsonl`, and an `_id`
that appears on more than one line — the same strictness the SQuAD path applies
to a record with no `answers`. #263's scope said `BeirAdapter` does not
change; this decision amends that scope to carry this reading path.
...
**When the walk fails, and the benchmark carries qrels (§ 5), the harness
returns a typed `HarnessError` of its own, distinct from `NothingToScore`.** The
walk fails when the generator's context comes from a node that is not a
`ContextBuilder`, or when the builder's chunks port — reached from a generator
or as the terminal node — names something that is not a node's `RankedChunks`
output entry: no node at all, a node with no entry in the trace, or a node whose
entry is of another kind. It **never falls back to `Context.chunks`**: that list
is cut to the builder's budget, so an nDCG over it would mean one thing under a
builder that keeps three chunks and another under one that keeps twenty.
`Context.chunks` stays available as a second ranking — what entered the prompt,
beside what retrieval offered — for a future per-node comparison; it is not the
...
assertion of the M3 calibration (#269).
...
`eval/ragondin-metrics/tests/` as the SciFact fixture is: a fixture covering the
generation subset below (N ≥ 1 000 queries) that holds, for each of its queries,
the ranking the run read under § 3, the answer **as text**, the qrels rows and
the reference answers needed to re-score it, and the re-scored exact match, F1
and retrieval values. The test re-scores the frozen answers and rankings against
the frozen qrels and references and requires the frozen values **exactly**. The
reference-answer text in that fixture carries the CC BY-SA 4.0 notice beside it.
The retrieval leg over the full question set is not frozen per query: it is
asserted by its aggregates in the calibration record, as M2's calibration
asserts its own.

**A rerun of the calibration is held to a tolerance, not to equality.** ADR-15
is scoped to judge-based runs, and there it answers non-determinism with
replicates and confidence intervals; its premise — that an LLM served with
dynamic batching is not deterministic even at a fixed seed — holds of a
generator as much as of a judge. The single-rerun tolerance here is this ADR's
own rule: rerun with the same model, temperature zero and a fixed seed, the mean
`exact_match` and the mean `token_f1` of the generation subset land within
**0.01 absolute** of the recorded means. The count of answers whose text changed
is **recorded, not bounded**. The rerun's retrieval metrics match the recorded
ones within `RECORDED_TOLERANCE`, and exactly on the recording machine, as M2's
calibration does: `RECORDED_TOLERANCE`'s doc comment in
`bin/ragondin/tests/calibration.rs` records that ONNX Runtime's summation order
moves the last bits of an `f64` on another machine.

**The calibration's generation leg may run over a documented, deterministic
subset** of the dev questions — the first N in file order, **N ≥ 1 000**, N
fixed and recorded by #269 — because 10 570 generations through a real model are
a budget the retrieval leg does not have. The floor is what gives the 0.01
tolerance meaning: at N ≥ 1 000 it allows up to ten flipped answers per
thousand, where over a few dozen questions one flipped answer would exceed it.
The retrieval leg runs over the whole set.

**The M3 exit criterion (#268) reuses the M2 curated fixture**,
`bin/ragondin/tests/fixtures/exit-criterion/`, and no SQuAD subset. That fixture
is a BEIR directory — graded qrels with explicit grade-0 rows, loaded as
`--benchmark beir/dataset` — which the SQuAD adapter cannot express. #268 adds
an `answers.jsonl` of hand-written reference answers to its dataset directory
(§ 2) and runs its generation configurations under
`--benchmark beir-qa/dataset`; the M2 runs, still loaded as `beir/dataset`,
stay reference-free and unchanged. The gap that criterion demonstrates is there
by construction, and nothing licensed enters the tree for it.

### 5. Identity and aggregation

**What *carries* means.** A benchmark **carries** a piece when at least one of
its queries has a non-empty list for it: at least one judgment (grade 0
included) for qrels, at least one string for reference answers. A `Benchmark`
built with an empty reference-answers value carries none. Every use of
*carries* for a benchmark and a piece in this ADR — the regime, the walk of § 3,
the refusal and the digest below — reads this definition. The query-level sense
it is built from is the plain one: a query carries a reference answer when its
list is non-empty, and qrels when it has at least one judgment.

**Reference answers enter `dataset_version` — only when the benchmark carries
them, behind a tag of their own.** Without them two benchmarks differing only in
references share one identity, and their runs collide. With them digested
unconditionally, every qrels-only benchmark's digest moves, including the two
`bin/ragondin/tests/calibration.rs` pins as `RECORDED_DATASET` and
`RECORDED_NFCORPUS_DATASET`. The qrels section stays as it is — unconditional, a
count followed by its rows, zero rows included — so a benchmark that does not
carry reference answers digests exactly as today, byte for byte. One that
carries them appends, after the qrels, a references section in the encoding the
rest of the digest already uses: the tag, the bytes `references` written as
every string is (its length first); then, as every existing sequence opens, the
count of the queries that follow; then, for each query in benchmark order that
has at least one reference, its query id, the count of its references, and the
reference strings in the order the benchmark holds them. A query with no
reference is absent from the section. `identity.rs` has no in-stream section tag
today — its sections are told apart by position and count — so this tag is the
first, and it exists because this section alone is conditional.

**Each metric averages over its own judged set**: exact match and F1 over the
queries with a non-empty reference list; the retrieval metrics over the queries
with at least one judgment. A query may be in one set, both or neither, and is
scored in each it belongs to and no other. The regime — which families a run
reports — is ADR-8's, read from the pieces the benchmark carries, and
`Benchmark` reports it (#263). Because a family is reported only when the
benchmark carries its piece, a reported family's judged set is never empty.
`HarnessError::NothingToScore` keeps its meaning: no family scored any query —
a mean over nothing, not a zero. A benchmark that carries reference answers and
no qrels is therefore a valid generation-only benchmark, and the walk of § 3 is
not attempted over it.
...
- **SQuAD v2.0's script.** Its normalisation is v1.1's, and its additions serve
  ...
  only correct answer. It also scores the both-empty case F1 1 where v1.1
  scores 0. Adopting it would change how three dev questions score an empty or
...
- **Scoring generation only once a pipeline ends in an answer.** It loses the
  regime with both pieces that ADR-8's typing implies, and with it the
  comparison of retrieval quality against answer quality query by query, which
  is what M3 exists to show.
...
- **The M3 evaluation issues are unblocked, and each owes this ADR something
  ...
  answers as a `Vec<String>` per query, the `BenchmarkError` for a question with
  no `answers`, the regime `Benchmark` reports, construction extended
  additively, as `Benchmark`'s documentation already provides for, the count and
  sample cross-check over a miniature fixture that is hand-written or a tiny
  subset carrying its licence notice, and the `answers.jsonl` reading path of
  § 2 under `--benchmark beir-qa/<dir>` — a change to `BeirAdapter` that #263's
  scope excluded and this decision amends that scope to include. #264:
  `exact_match` and `token_f1` in `ragondin-metrics`, on the [0, 1] scale, with
  the parity fixture and every edge case § 1 lists, regenerated by a recorded
  script calling the pinned SQuAD script. #265: regime selection in the harness,
  the port walk of § 3 and its typed error, the typed error of § 5 for a
  benchmark with reference answers run through a pipeline that produces no
  `Answer`, a separate denominator per metric family, and the `dataset_version`
  tag of § 5 — #265's because the harness owns `identity.rs`, although ADR-C29's
  Consequences named #263 for that obligation. #266: the binary's `--benchmark`
  selector — `benchmark_root` in `bin/ragondin/src/bench.rs`, which accepts only
  `beir` today and says so — extended to `squad/` and `beir-qa/`; #263 is scoped
  to `ragondin-benchmarks` and never touches the binary. #268: an
  `answers.jsonl` of hand-written reference answers in the M2 exit-criterion
  dataset, and its generation configurations run under `beir-qa/dataset` while
  the M2 runs stay under `beir/dataset`. #269: the SciFact assertion through the
  new read path, the SQuAD calibration, the subset rule with its floor of
  N ≥ 1 000, the tolerances of § 4, and the frozen fixture with its licence
  notice. None of them reopens this decision; each implements a named part of it.
...
- **The invariants.** INV-1 is untouched: no type in `ragondin-types`,
  ...
  ADR-10 is extended, not reopened. ADR-15's premise — no determinism at a fixed
  seed — is applied to a generator; the single-rerun tolerance is this ADR's own
  rule, not ADR-15's replicates. ADR-C28 and ADR-C29 are what make § 3 a lookup
  rather than a computation: the trace names each node's ranking, and the two
  generation nodes fix which port is which.
