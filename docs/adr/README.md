...
3. **A `decision` issue produces exactly one ADR — when it decides how the system is built.** Architectural questions that an agent must not answer alone (see `docs/OPEN_QUESTIONS.md` and the `opening-a-decision-issue` skill) are resolved in a `decision` issue, and the outcome is a single new accepted ADR. A decision about the repository's own **governance** — where a rule is stated, how an existing invariant is mechanized, what the permanent record may contain — produces no ADR: it resolves by amending the rule where it is stated, in `AGENTS.md` or in this file, and the resolving PR names the decision issue it closes. The test that tells the two apart: *would the answer change what the code does, or what a component author can rely on?* Yes → an ADR. No → the rule's home. Three decisions established this before it was written down — #73 (attribution in git history, PR #86), #146 (the scope of INV-6's mechanized check, PR #185) and #66 (where the binding rules live, PR #186) — each recorded as a "waiver" of this rule on the previous one's precedent. It is a rule now so that the fourth does not have to argue from precedent.
...
- **`ADR-C01-*.md` … `ADR-C18-*.md`** — the frozen decisions of the *code* architecture, one file per decision, cited as `ADR-C1` … `ADR-C18`.
...
- Code-architecture decisions use the `C` prefix: `ADR-C1` … `ADR-C18`.
