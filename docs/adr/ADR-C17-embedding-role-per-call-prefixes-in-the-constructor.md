...
**The embedder is not a pipeline node.** `ragondin-engine`'s `ComponentFamily` is deliberately wider than `LogicalNode`'s variants: an `Embedder` and a `VectorStore` are components a dense retriever is *built from*, resolved through the registry so that the retriever's constructor can reach them, and `ResolvedComponent` has no variant for either. `EmbedParams` is therefore never populated from a node's parameter map. `embed` has exactly two callers, both of them Rust: `ragondin-retriever-dense` (#23), which embeds a query, and the harness and composition root (#29, #31), which embed a corpus — a path #23 keeps deliberately outside the pipeline formalism so as not to touch open question 5. **Each of the two knows its side statically, for free.**
...
- **The role is constructor configuration; one model registers twice** (`impl: bge_query` and `impl: bge_passage`, two entries over the same weights). It does not remove the silent failure; it moves it into the configuration, where nothing checks it — wiring the passage-side entry into the query side parses, plans, runs, and quietly loses the points. It also forces **two** `Embedder`s into `ragondin-retriever-dense` (#23), a shape that issue does not anticipate; it costs a second model session over one set of weights; and it makes a run's `model_hashes` (`docs/system-architecture.md` §7.1) report two models where there is one.
