...
| `ragondin-proto` | The protobuf face of the component contract (package `ragondin.v1`): the `Retriever`, `Fusion`, `Reranker`, `Embedder`, `VectorStore`, `ContextBuilder` and `Generator` services, with `GetModelIdentity` on every model-bearing one, stubs generated at build time with no `protoc` (ADR-C34), and a reserved, empty configuration-delivery service. |
| `ragondin-remote` | The `Remote` adapters for the `Retriever`, `Fusion`, `Reranker`, `Embedder`, `VectorStore`, `ContextBuilder` and `Generator` families over gRPC, the domain ⇄ protobuf conversions with their round-trip tests, and the one status ⇄ `ComponentError` mapping (ADR-C35). Nothing binds a `Remote` component into the binary yet. |
...
Still a compiling skeleton, with a doc comment and a link test and no
behaviour: `ragondin-server`, the serving driver, waiting on its own issue. A
`VectorStore` over Qdrant is waiting on one too.
