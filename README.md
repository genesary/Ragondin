...
| `ragondin-remote` | The `Remote` adapters for the `Retriever`, `Fusion`, `Reranker`, `Embedder` and `VectorStore` families over gRPC, the domain ⇄ protobuf conversions with their round-trip tests, and the one status ⇄ `ComponentError` mapping (ADR-C35). Nothing binds a `Remote` component into the binary yet. |
