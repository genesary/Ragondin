...
//!   component implements — the protobuf mirror of the `rag-contracts` traits;
//! - the **configuration-delivery service** (the [`config`] module): a
//!   purpose-built, versioned gRPC service with ACK/NACK.
//!
//! The domain types in `rag-types` are the **source of truth** (INV-9); the
//! protobuf here is generated to mirror them, and `rag-remote` supplies the
