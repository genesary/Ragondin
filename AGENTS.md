...
- **Skills:** `.claude/skills/` holds only skills written for this repository. An external skill is **never copied in** — verbatim, trimmed, or lightly edited — because a skill is read by an agent as governance, in the same context as this file, and a text written elsewhere can contradict it without anyone having decided that it should. A skill **derived** from an external text is allowed on three conditions: it is rewritten in this repository's voice and against this file's rules, so that it states *these* rules and cannot quietly disagree with them; its front-matter carries a `derived-from:` field naming the source (URL and revision, or title and author) and the source's licence, which must permit a derivative under Apache-2.0; and it is reviewed as a rule change, not as a file — a skill's failure mode is behavioural and shows up in a later PR, not on a skim. Nothing here governs what a harness loads from outside the tree: that is not reproducible from a checkout, and only what is committed is. *Decided in #67.*
