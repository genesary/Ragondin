...
The **How** column is load-bearing, because two different things are called "CI-enforced" here:
...
| **INV-6** | **No global state.** The component registry lives on an `EngineContext` passed explicitly as a parameter. Never use a static global registry (`inventory`, `linkme`, or equivalent). *Argued in [ADR-C4](docs/adr/ADR-C04-engine-as-embeddable-library-with-explicit-context.md).* | Closure (`inventory`, `linkme`) |
...
- **INV-6** — CI decides `inventory` and `linkme`. *Or equivalent* is a judgment: the sign is any registration that reaches the registry without an `EngineContext` parameter — a `static` registry, a link-time collected slice, or a `register()` free function with no context argument.
...
*Argued in* means the ADR's own `## Decision` states the invariant's content. *Partly argued in* means it grounds some of the rule and not the rest, and says which. **INV-1 and INV-2 have no grounding ADR at all** — the stable/internal split is in force and enforced, but was never argued anywhere; #72 records it. That gap is written into the table rather than left blank, because a blank cell reads as unfinished work while an explicit "none" is a finding.
