...
# Every recipe above that resolves or builds `--all-features` -- test-features,
# check-features, clippy, doc, check-deny -- runs `ort-sys`'s build script,
# which fetches a prebuilt ONNX Runtime (a static archive, SHA-256 pinned,
# from cdn.pyke.io) into the user cache -- ~/.cache/ort.pyke.io on Linux,
# ~/Library/Caches/ort.pyke.io on macOS -- once per machine and target. That
# archive is outside what check-deny audits. Offline or air-gapped: point
# ORT_LIB_LOCATION at a directory holding libonnxruntime.a and set
# ORT_SKIP_DOWNLOAD=1. Decided in ADR-C27.
...
| **INV-4** | **The core stays light.** The whole of `core/` — `ragondin-types`, `ragondin-pipeline`, `ragondin-contracts` — must carry **no heavy dependency** anywhere in its closure: no retrieval engine, no RPC or HTTP stack, no inference runtime, no vector-store client. `serde` at most. **Which crates those are is decided by `DENY_EXACT`/`DENY_PREFIX` in [`scripts/check-invariants.py`](scripts/check-invariants.py), and maintained there** — a new heavy backend is added to that list, not to this row. *Partly argued in [ADR-C14](docs/adr/ADR-C14-heavy-backends-feature-gated-lean-default-build.md) (heavy dependencies are confined to component crates, behind features); the rule about the core is not stated there.* | Closure |
...
- If a task requires an architectural choice that is **not** already settled here or in `docs/`, what you do is decided by **where the choice lands**, never by how reversible you judge it to be — *decided in #63 and #65*:
  - **It escalates — stop, open a `decision` issue, and do not proceed — when it reaches a shared surface**: a change to the public API of `ragondin-types`, `ragondin-pipeline` or `ragondin-contracts` (INV-1); the wire format (INV-9), and anything under `wire/` a `Remote` service compiles against ([ADR-3](docs/adr/ADR-003-two-faced-component-contract-local-remote.md)); the public surface of `ragondin-engine`, or what a driver observes of its execution — how a node is resolved at planning, what the trace carries, the order in which nodes run — since INV-2 makes the engine's internals cheap to change, not its design one implementer's call, and a refactor that changes none of these is INV-2's business and never escalates; a new `[workspace.dependencies]` entry that duplicates a **utility** role the table already fills (a second HTTP client, serializer or async runtime), or that a crate outside `components/` depends on in the same diff — a second **backend** of a family already served, confined behind its own component's feature, is not a duplicate ([ADR-C14](docs/adr/ADR-C14-heavy-backends-feature-gated-lean-default-build.md)); the **feature list of an existing `[workspace.dependencies]` entry** — one line that every crate naming the entry inherits and that Cargo unifies across them, so adding to it or changing it, at the root or by appending features on a member's own `workspace = true` line, is a change to every one of those crates and never a leaf's choice ([ADR-C27](docs/adr/ADR-C27-onnx-runtime-obtained-by-download-binaries.md), *decided in #223*); a frozen decision; or an entry in `docs/OPEN_QUESTIONS.md`. That list is the whole boundary. It is checked against the diff, not forecast from blast radius. It answers *which choice you may not make alone*, and it is neither of the two lists it resembles: not INV-1's stable API boundaries, which answer what an outside consumer may depend on, and not `docs/AGENT_WORKFLOW.md`'s list of PRs a human reviews closely, which weighs in-workspace blast radius and therefore names `ragondin-engine` — a crate INV-2 says is not and will never be a boundary. The three are compared side by side in `docs/AGENT_WORKFLOW.md` § When to escalate to a human. What escalates then resolves under `docs/adr/README.md` process rule 3.
  - **Otherwise it is yours to make** — typically because it stays inside one crate: which tokenizer, how a knob is exposed, which backend to confine behind your crate's own feature ([ADR-C14](docs/adr/ADR-C14-heavy-backends-feature-gated-lean-default-build.md)). A choice made this way is **recorded, never silent**: argued in the crate's `ARCHITECTURE.md` (create it if the crate has none), and named in the PR description under its own heading so the reviewer sees it. A choice made in a leaf is still a precedent, and a reviewer can only object to one that was named. Passing the conformance suite is not that record: it gates behaviour, not design.
  - **Reversibility is not the test**, in either direction. An agent is a poor judge of its own blast radius, and a diff is checkable where a forecast is not. A reversible choice on a shared surface still escalates; an irreversible-looking one inside a leaf is still yours, and the record is what lets the next reader disagree with it.
...
- **Skills:** `.claude/skills/` holds only skills written for this repository — original, or derived under the conditions below. An external skill is **never copied in** — verbatim, trimmed, or lightly edited — because a skill is read by an agent as governance, in the same context as this file, and a text written elsewhere can contradict it without anyone having decided that it should. A skill **derived** from an external text is allowed on these conditions, and with the consequence that follows them:
  ...
  - It is reviewed for what it would make an agent do — against this file, and against § Rules here, procedures in the skills — never skimmed as a file: a skill's failure mode is behavioural and shows up in a later PR.
  - It is owned here. An upstream change creates no obligation, and a later re-derivation updates the revision.

  `derived-from:` is provenance, not attribution of the work: it names a source the way a citation does. The Attribution rule above does not reach that field, nor the one sentence of a PR description that points at it; it reaches everything else exactly as written — a commit message names the skill, never the source. Nothing here governs what a harness loads from outside the tree — that is not reproducible from a checkout — and nothing loaded that way outranks this file: where an external skill and this file disagree, this file binds — *decided in #67*.
