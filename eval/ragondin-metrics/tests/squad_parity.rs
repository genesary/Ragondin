...
/// a backslash, `\u{hex}` a character, and a field that is exactly `\e` the
/// empty string.
...
fn parse(line: &str) -> Case<'_> {
    // An empty field is escaped as `\e`, so a line never ends in a tab. One
    // that does was edited by hand or regenerated by an older script, and an
    // editor stripping that tab would silently drop a reference.
    assert!(
        !line.ends_with(char::is_whitespace),
        "fixture line ends in whitespace, which an editor may strip; an empty \
         field must be written `\\e`: {line:?}"
    );
...
fn cases() -> Vec<Case<'static>> {
    // Lines are not trimmed. The fixture escapes a space at either end of a
    // field and writes an empty field as `\e`, so no line ends in whitespace
    // and `parse` refuses one that does rather than lose a field to it.
    FIXTURE
        .lines()
